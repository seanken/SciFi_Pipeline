...
#bustools text $outdir/output.bus | awk '{print $1}' | sort | uniq  > $outdir/whitelist.unfilt.txt
#bustools text -p $outdir/output.bus | awk '{print $1}' | sort | uniq -c | sort | tail -n 100000 | awk '{print $2}' > $outdir/whitelist.unfilt.txt
#python MakeWhitelist.py  $outdir/whitelist.unfilt.txt $outdir/whielist.txt

echo Extract Counts!
bustools text -p $outdir/output.bus | awk '{print $1}' | sort | uniq -c | sort > $outdir/read.counts.txt 
echo Make list!
python MakeWhiteListSimple.py $outdir/read.counts.txt $outdir/whitelist.init.txt
python MakeWhitelist.py $outdir/whitelist.init.txt $outdir/whitelist.txt
...
bustools text -p $outdir/corrected.sorted.bus | python FilterUMI.py $outdir/filt.bus | sort | uniq -c | sed 1d > $outdir/umi.counts.txt
